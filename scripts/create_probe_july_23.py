...
PROBE_FILE = OUTPUT_DIR / "submission_probe_july_23.csv"
...
if SAMPLE_SUB.exists():
    ...
    # Mask: July 2023
    july_23_mask = (df['Date'].dt.year == 2023) & (df['Date'].dt.month == 7)
    
    # Set probe values
    df['Revenue'] = 0.0
    df['COGS'] = 0.0
    df.loc[july_23_mask, 'Revenue'] = C
    
    # Save
    df.to_csv(PROBE_FILE, index=False)
    
    print(f"Revenue Probe for July 2023 created at: {PROBE_FILE}")
    print(f"Probing constant C: {C:,}")
    print(f"Number of targets in block: {july_23_mask.sum()}")
else:
