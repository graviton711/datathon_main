...
PROBE_FILE = OUTPUT_DIR / "submission_probe_feb23.csv"
...
if SAMPLE_SUB.exists():
    ...
    # Mask: February 2023
    feb23_mask = (df['Date'] >= '2023-02-01') & (df['Date'] <= '2023-02-28')
    
    # Set probe values
    df['Revenue'] = 0.0
    df['COGS'] = 0.0
    df.loc[feb23_mask, 'Revenue'] = C
    
    # Save
    df.to_csv(PROBE_FILE, index=False)
    
    n_days = feb23_mask.sum()
    
    print(f"Probe for February 2023 created at: {PROBE_FILE}")
